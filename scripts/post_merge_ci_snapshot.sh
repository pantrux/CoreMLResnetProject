...
api "/actions/workflows/ci-liveness.yml/runs?per_page=20" > "$runs_liveness_json"
api "/actions/workflows/ios-build.yml/runs?per_page=20" > "$runs_ios_json"
...
if [[ "$liveness_push_for_sha" -lt 1 || "$ios_push_for_sha" -lt 1 ]]; then
